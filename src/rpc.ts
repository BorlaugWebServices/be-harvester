...
export const server = jayson.server({
    ...
    getDIDState: async function ({did}, callback) {
        debug(`getDIDState: ${did}`);

        try {
            if (!blockProcessor.api) {
                blockProcessor.api = await ApiSingleton.getInstance();
            }
            // if (blockProcessor.api.query.identity) {
            //     const identityQueries = Object.keys(blockProcessor.api.query.identity);
            //     debug(`Valid identity queries: ${identityQueries.join(', ')}`);
            // }
            const properties = [];
            if (ApiSingleton.isMaya) {
                const all_entries = await blockProcessor.api.query.identity.didInfo(did);
                const didInfo = all_entries.toHuman() as unknown as any;
                didInfo.properties.forEach(property => {
                    properties.push({
                        name: property.name,
            ...
            const claims = [];
            all_claim_entries.forEach(([[,], value]) => {
